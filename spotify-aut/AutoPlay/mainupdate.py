...
DAY = 300 #5min in seconds
MIN_DAY_PLAY_DURATION = 120 # 2min
MAX_DAY_PLAY_DURATION = 240 # 4mins
BREAK_DURATION = 30 # 30 spread over 3 times a day
...
def play_playlist(playlist,device_id, playlist_type = "user"):
    if playlist_type == "user":
        for ids in playlist:
            user_playlist(ids, device_id)
    else:
        for ids in playlist:
            artist_playlist(ids,device_id)

def user_playlist(ids, device_id):
    playlistID = f"spotify:user:{credentials['USERNAME']}:playlist:{ids}"#play song
    print("playlist: ",playlistID, "device id: ", device_id)
...
def artist_playlist(ids, device_id):
...
def generate_play_duration():
    ...
    random_break = generate(15,max_break_time) #between 5s to 30s
...
def check_break_time():
    ...
    if get_current_time() >= (play_duration+start_time):
        #take a break
        sp.pause_playback()
        print("141 random_break_end:",convert(random_break_end))
    ...
    if get_current_time() <= end_time: #always compair the current time with expected daily end time
        ...
        if (play_duration+start_time) > start_time and play_duration > PLAY_TIME_DURATION: #check if play durations > 30 secs
            ...
            if playlist_ids != 0 and artist_ids != 0:
                ...
                play_playlist(playlist_type="user",device_id=str(credentials["DEVICE_ID"]), playlist=playlist["PLAYLIST_IDS"])
                    # if PLAY_TIME_DURATION >= break_time_counter:
                    #     #choose to play next truck or take break
                print('playing artist')
                play_playlist(playlist_type="artist",device_id=str(credentials["DEVICE_ID"]), playlist=playlist["ARTIST"])
                ...
                break_time_counter += 1
                play_playlist(playlist_type="user",device_id=str(credentials["DEVICE_ID"]), playlist=playlist["ARTIST"])
        else:
        ...
        sp.pause_playback(device_id=str(credentials["DEVICE_ID"]))    
